...
//	}
	...
	if($res&&$pid){
		...
		foreach($_POST as $k=>$post){
			if(strpos($k,"_") ){       			//查找 "_" 在字符串中第一次出现的位置：
				$str=explode("_",$k,2);			//把字符串打散为数组,返回两个数组
				$array[$str[0]][$str[1]]=$post;
			}
		//	echo $k;
		}
	//	echo "<br>";
	//	var_dump($array);
		foreach($array as $k=>$val){
			$array_config = array();
			$array_config['p_id']=$pid;
			$array_config['term_id']=$k;
			$array_config['selection_one']=$val['1'];
			$array_config['selection_two']=$val['2'];
			if(isset($val['3'])){
				$array_config['selection_three']=$val['3'];
			}
			if(isset($val['4'])){
			$array_config['selection_four']=$val['4'];
			}
			if(isset($val['5'])){
			$array_config['selection_five']=$val['5'];
			}
			if(isset($val['6'])){
			$array_config['selection_six']=$val['6'];
			}
		//	echo "<br>循环数组";
		//	var_dump($array_config);
			insert("imooc_config",$array_config);
...
//		print("1");
		...
		foreach($array as $k=>$val){
			...
			//print_r("123");
			//print_r($k);
			update("imooc_config",$array_config,"p_id={$id} and term_id={$k}");
		}
